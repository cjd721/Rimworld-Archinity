...
**Glitterite stronghold**:
A Glitterite site the campaign's raids are aimed at, generated by the quest that needs it
and holding that quest's goal. Not their ordinary **settlements**, which stand on the
orbital map from the reveal and behave as any vanilla settlement does.
[Glittertech](docs/requirements/GLITTERTECH.md)
_Avoid_: holding (a vassal's land), outpost (a site the colony staffs with its own people)

**The reveal**:
The single campaign moment that opens orbit, earned from the planetary resolution. Before
it there is no view of the orbital map, no flight and no orbital sites; after it the
orbital board is visible and populated. [Space](docs/requirements/SPACE.md)
