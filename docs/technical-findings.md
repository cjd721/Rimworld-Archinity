...
From [How the world starts, and how it changes](https://github.com/cjd721/Rimworld-Archinity/issues/8).
All decompiled from `Assembly-CSharp.dll` (1.6.4566) unless noted. These extend
`## Factions` above rather than replacing it; the roster-freeze finding there is
unchanged.
...
- **Layout selection reads the current def at map-generation time**, and
  ...
  Three distinct sites, which an earlier draft conflated:
...
| Leak | Detail | Fix |
|---|---|---|
| `Settlement.cachedMat` | caches texture **and** colour from the def on first draw (`Settlement.cs:20,68-78`); **never nulled anywhere in the assembly**, not scribed | null by reflection |
...
| `FactionUtility.DefaultFactionFrom` | resolves `AllFactions.Where(x => x.def == ft)`, falling back to `replacesFaction`, else **null** — orphans any `PawnKindDef.defaultFactionDef` | set `replacesFaction` on higher tiers pointing down the chain |
...
it. **`Settlement.previouslyGeneratedInhabitants` is a non-hazard in 1.6, and this
resolves an earlier "plausible but unconfirmed".** The read path
(`PawnGenerator.cs:209-219`) is real, but the **only add site**
(`:235-237`) is
...
## Silent failures found this session

1. **`maxCountAtGameStart` and `canMakeRandomly` are `[Obsolete]` and read
   nowhere in 1.6** (`FactionDef.cs:239-243`). No Core/DLC def sets either. **Six
   mods in the bin still do**, writing to no-ops with no warning. The live
   replacements are `maxConfigurableAtWorldCreation` and
   `startingCountAtWorldCreation`.
2. **Writing `FactionDef.techLevel` at runtime silently reverts on load.** Defs
   are not scribed, so the value is gone next session with no error and no log
   line. It also only changes the tech level — pawn kinds, traders and KCSG
   layouts are unaffected — and, because a `FactionDef` is a shared object,
   changes it for **every** faction instance using that def. WTL does exactly
   this in production (`Patch_BaseGen.cs`, prefix + `[HarmonyFinalizer]`), which
   will also clobber any write made during a BaseGen pass.
3. **`Settlement.cachedMat` is never invalidated** — see the table above.
4. **The `VFEE_Deserters` faction blacklists every other raid strategy**, in a
   `[StaticConstructorOnStartup]` that sets
   `disallowedRaidStrategies = AllDefs.Except(VFEE_DefOf.DesertersStrat)` — no
   filter, one exclusion. **Any `RaidStrategyDef` we author is silently excluded
   from that faction.** Two corrections to how this was first recorded: the code
   is in **VFE *Empire*** (`2938820380`), not VFE Deserters, in
   `VFEEmpire.RaidStrategyWorker_Deserters`; and `disallowedRaidStrategies` is a
   **vanilla `FactionDef` field** (`FactionDef.cs:178`), consumed at
   `RaidStrategyWorker.cs:54` — not a VFE extension. Two escapes exist: a custom
   `workerClass` whose `CanUseWith` does not chain to base ignores the list
   entirely, and `IncidentWorker_RaidEnemy.ResolveRaidStrategy` (`:94`) only
   filters when `parms.raidStrategy == null`, so a pre-set strategy bypasses it.
5. **`FactionUtility.DefaultFactionFrom` returns null** once every faction has
   climbed off a def — see the table above.
6. **`RaidStrategyDef` and `QuestScriptDef` have no `minTechLevel` field at
   all.** Verified against the field lists. Every tech gate in either is worker
   code. `QuestScriptDef`'s available gates are `rootMinPoints`,
   `rootMinProgressScore`, `rootEarliestDay`, `rootSelectionWeight`,
   `isRootSpecial` and `randomlySelectable`. There is nothing to validate against
   in XML.
7. **`VFET_OpportunitySite_WildMen` generates a faction at runtime** —
   ...
   `Find.FactionManager.Add`, with relations computed from
   `AllFactionsListForReading` at that moment (`:39-58`). **There is no reuse
   guard** — `FirstFactionOfDef` appears nowhere in the assembly, so it can fire
   more than once. It only fires for a Neolithic player (`:149`), which is
   precisely our Neolithic. *Correction to the earlier note: the generated
   faction is **Hostile** to the player (`kind = 0`), not neutral.* A frozen
   roster cannot survive it.

## Smaller faction facts

- **`DankPyon_MedievalSiege` cannot fire as shipped.** Its worker requires
  `RaidStrategyWorker_Siege.CanUseWith` (i.e. `FactionDef.canSiege`) **and** a
  `MedievalOverhaul.FactionSiegeExtension` with `medievalSiege: true` **and**
  `techLevel == Medieval`. The extension is on `DankPyon_BrigandFactionBase` and
  `DankPyon_NobleHouseFactionBase`; **neither sets `canSiege`**, and `canSiege`
  has **zero hits across all of Medieval Overhaul** (1.4/1.5/1.6, defs, patches
  and assembly). Core `FactionBase` does not set it either and the field defaults
  `false`. Empty intersection.
  **Fixing it is not a one-liner:** `canSiege` must go onto **both** abstract
  defs, and `RaidStrategyWorker.CanUseWith` adds further gates — the def's curve
  is `(500,0) → (1000,1.6)`, so it needs **>500 threat points**, plus the Surface
  layer and no blocking tile mutator. That makes it *possible*, not frequent.
  The def already ships `LordJob_MedievalSiege` with its own supply generation
  and **trebuchets** — `DankPyon_Turret_Trebuchet` is the only holder of
  `ArtilleryMedieval_BaseDestroyer`. *(MO's own arrival text says "catapults";
  that is flavour copy, not the def.)*
- **`VFET_WildMen` is a *player* faction** (`isPlayer true`). The NPC def is
  ...
  are the obsolete no-ops in silent failure #1** — so it stays out by accident,
  not by design.
...
- ⚠️ **Duplicate installs on disk, and the scale is the point: 76 of the 81
  workshop-id folders have a full second copy** under
  `steamapps/common/RimWorld/Mods/`, declaring the same `packageId`. These are
  real copies, not junctions — VEF (`2023507013`) is 13 MB in both. Five were
  diffed and are byte-identical; the other 71 were not. **This must be resolved
