...
| `vanillaracesexpanded.saurid` | ~~`replacesFaction` **deletes a vanilla faction**. Patch it out before worldgen.~~ **STRUCK — this was wrong. [V]** `replacesFaction` is a **vanilla Biotech mechanism**; four of the five uses in the current set are vanilla's own (`TribeRoughNeanderthal`→`TribeRough`, `TribeSavageImpid`→`TribeSavage`, `OutlanderRoughPig`→`OutlanderRough`, `PirateWaster`→`Pirate`). Decompiled `RimWorld.FactionGenerator`: it only suppresses a faction from the **auto-added required-at-game-start list** (`!Any(x => x.requiredCountAtGameStart > 0 && x.replacesFaction == facDef)`); the `FactionDef` survives and remains selectable on the world-creation screen. With Biotech on, pigskins were **already** replacing plain rough outlanders before Saurid was installed. Saurid adds a second replacer for the same slot, neither replacer is itself replaced, so **both are added and the roster gains a faction**. Nothing to patch, no deadline. |
| `vanillaracesexpanded.waster` | Rewrites the vanilla Waster xenotype; `Rand` in a render path. |
| `vanillaracesexpanded.android` | Patches the **abstract** outlander and pirate bases — but **the scale matters and was never stated. [V]** `1.6/Patches/FactionPatches.xml` in full is three `PatchOperationAdd`s appending one line, `<VREA_AndroidAwakened>0.02</VREA_AndroidAwakened>`, to the `xenotypeChances` of `OutlanderFactionBase`, `PirateBandBase` and `Empire`. A 2% chance, read at **pawn** generation, so it is reversible at any time and is not a worldgen decision. Scope it if you want zero androids in outlander raids; ignore it otherwise. Its `AndroidSettings.xml` is still the model for Def-based tunables. |
