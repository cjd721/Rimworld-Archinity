# Cosmology in practice

How the fiction in `COSMOLOGY.md` reaches the player, and the small number of
ways it can be destroyed.

**Scope.** Cosmology is what is true. This is what to do about it. Every rule
here is downstream of a fact there, and the fact is cited rather than restated.

**Read this before writing a quest, a letter, an ideology, or a system that
touches the altar.** Several of these rules fail silently: the game runs, the
content ships, and the thing that made it worth doing is quietly gone.

---

## 1. Nothing is ever stated plainly

**No letter, no quest description, no research blurb explains the tower, the
experiment, or the Archons' purpose.** The moment the fiction confirms its own
premise, the best thing about it dies. The player is meant to assemble a version
of the cosmology and be permanently unsure whether they got it right.

It arrives only as **anomaly** — things that behave like a system rather than a
world:

- Something repeats with different names, and someone notices.
- An Archon says one sentence implying they have run this before, does not
  elaborate, and is gone.
- The Glitterites' instruction is the loudest evidence in the game. Somebody had
  to *set a rule* about what may leave a universe, and rules imply an author.

**The question of whether this reality is real stays open permanently.** It feels
real. It is the realest thing anyone in it has ever known. And there is something
behind it the player can sense and never verify. Two players arguing about it at
three in the morning, both wrong in different ways, is the target state.
...
campaign wants them doing. Waystone's *design against the min-maxer* is satisfied
here rather than violated: the min-maxed outcome is a colony that built a real
...
**Habituation is a thing the player feels, never a thing the game measures.** The
line about nobody reading the names on the intake list is a description of what
six hundred days does to a person at the keyboard. It is delivered by pacing and
...
The fiction is mostly a reskin. Waystone's *reskin before you rebuild* applies
with unusual force here, because several things already in the load order are
already correct once renamed.

- **Anima trees, anima grass, anima linking.** Named for the substance, not the
...
Waystone governs the design where the two documents meet. Two of its rules are
consequences of the cosmology rather than independent assertions, and should not
be re-argued on their own terms:

- **_Efficiency never means mercy, it means scale_** is Cosmology §6. The rate
  improving and the bill exploding are the same fact.
- **_One machine, one philosophy_** is Cosmology §3. A rival device that buys
  power without a life would have to source anima from something that was never
  seeded, and there is no such thing.

## 8. Known soft spots

Not a to-do list. Places where this document is thinner than it looks.

**Habituation has no carrier.** It is the campaign's stated real antagonist and it
is delivered entirely by pacing and prose, which means it is delivered by whoever
writes the late-campaign beats and by nothing else. If those beats do not land it,
nothing else in the project will.

**The second rival faction is unwritten**, along with the shape of the tension
between the two climbers. Cosmology holds it open deliberately. Do not invent it
in a quest.
