...
The expanded hacking system becomes a core Ultra pillar. Intel does not only explain armor and reactors; it teaches the colony the protocol language spoken by Glitterite technology. Named campaign artifacts can unlock qualitative hacking research: new augments, remote intrusion capability and access to classes of systems that were previously impossible to touch.
...
Rising Trace: the same attacker is correlated across facilities; raids and pursuit become more frequent and dangerous.

High Trace: the gravship and its signatures are understood; Glitterites actively hunt the colony across space.
...
Glitterite places come in two kinds, and the words are vanilla's wherever vanilla has one.

**Settlements** are the ordinary kind: Glitterite holdings that stand on the orbital map
from the reveal, visible alongside every other power's, hostile to everyone and outside
...
destroyed, taken or made a vassal stays that way.
...
- **The objective** — an Archotech object, the analytical key, the protected persona
  records, whatever that raid is for.
- **Intel sources** — laboratories and records the raid can observe.
- **Artifacts to carry home** — devices for destructive analysis, protocol artifacts,
  surviving exemplars.
- **Lore to inspect** — murals, terminals, persona records, environmental evidence, for
  the player who is curious.
- **Layered defenses** — hackable doors and systems, turrets, mechanoids. The final
  stronghold adds a boss-scale mech embodying its local leader.
- **Enemies** — defenders present in the map, not merely defenses.
- **A hostile environment** — vacuum, sealed sections and breaches are part of the heist.

Two rules hold over all of it. **A quest-critical goal is always generated, reachable and
obtainable** — a stronghold whose objective failed to generate, or generated sealed away,
is a broken story line, not a hard raid. And **strongholds differ from one another**;
not every one holds a guaranteed item, but none is a copy of the last.
