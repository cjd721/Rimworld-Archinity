# Superseded build — the altar authoring a custom Transcendent gene

> Archived from [`docs/specs/ALTAR.md`](../specs/ALTAR.md): the final rite grants the shipped `VRE_Transcendent` gene, with no authored per-founder gene and no return after death ([`plot/ENDING.md`](../plot/ENDING.md) § *The Final Altar Ritual*; [`COSMOLOGY.md`](../COSMOLOGY.md) § *The complete operation*), so this build (#59) is dead. Its live engine facts are in [`docs/engine/def-loading.md`](../engine/def-loading.md) and [`docs/engine/health-and-death.md`](../engine/health-and-death.md); the live rite is `ALTAR.md` § *The final rite*. History, not authority.
